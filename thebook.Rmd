...
plot = ggplot(killsPerYear, aes(x=année,y=morts))+geom_line()+ggtitle("Evolution du nombre de victimes du terrorisme dans le monde.")+xlab("Année")+ylab("Nombre de morts")+theme(axis.text.x = element_text(angle=180))
...
plot = ggplot(killsPerRegion, aes(x=reorder(région,-morts),y=morts))+geom_bar(stat="identity")+ggtitle("Nombre de victimes du terrorisme par région")+xlab("Région")+ylab("Nombre de morts")+theme(axis.text.x = element_text(angle=45))
ggsave(plot, filename = "graphiques/killsPerRegion.pdf")
...
plot = ggplot(killsPerCountry, aes(x=reorder(pays,-morts),y=morts))+geom_bar(stat="identity")+ggtitle("Les dix pays les plus meurtriers.")+xlab("Pays")+ylab("Nombre de morts")+theme(axis.text.x = element_text(angle=45))
ggsave(plot, filename = "graphiques/tenDeadliestCountries.pdf")
...
ggsave(plot, filename = "graphiques/successRatioPerYear.pdf")
...
mostTargetedNationality = ddply(mostTargetedNationality, .(année, nationalité_ciblée), summarize, morts = max(na.omit(morts)))
